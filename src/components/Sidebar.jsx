...
import { visibleGroups, HR_ROLES } from '../config/navigation'
    ...
    className={({ isActive }) =>
      cx(
        ...
        isActive
          ...
          : 'font-normal text-slate-100 hover:bg-white/[0.08] hover:text-white'
      )
    }
  >
    {({ isActive }) => (
      <>
        {/* Amber mark on the active rail */}
        <span
          className={cx(
            'absolute left-0 top-1/2 h-5 w-[3.5px] -translate-y-1/2 rounded-r-full bg-amber-400 transition-all duration-300 shadow-xs',
        ...
        {collapsed && (
          <span className="pointer-events-none absolute left-full z-50 ml-3 hidden whitespace-nowrap rounded-md bg-slate-900 px-2.5 py-1.5 text-xs font-semibold text-white opacity-0 shadow-lg ring-1 ring-white/10 transition-opacity duration-150 group-hover:opacity-100 lg:block">
            {item.label}
          </span>
        )}
      </>
      ...
      >
          ...
          {collapsed ? (
            ...
            <div className="flex min-w-0 items-center gap-2">
              <img
                src="/truckmit-logo-white.png"
                alt="TruckMit"
                className="h-9 w-auto max-w-[190px] object-contain drop-shadow-xs"
                onError={(e) => {
                  e.target.onerror = null
                  e.target.src = '/logo-white.png'
                }}
        ...
        <nav className="custom-scrollbar flex-1 space-y-6 overflow-y-auto px-3 py-5">
          {groups.map((group) => (
            <div key={group.label} className="space-y-1">
              {!collapsed && (
                <p className="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-slate-300/80">
                  {group.label}
                </p>
              )}
        ...
        <div className="shrink-0 border-t border-white/[0.06] p-3">
          {collapsed ? (
            ...
            <>
              <div className="flex items-center gap-3 rounded-xl bg-white/[0.07] p-2.5 ring-1 ring-inset ring-white/[0.08]">
                <Avatar name={user?.full_name || user?.name} src={user?.photo_path} size="sm" />
                ...
                <button
                  onClick={logout}
                  className="rounded-md p-1.5 text-slate-300 transition-colors hover:bg-red-500/20 hover:text-red-300"
                  title="Sign out"
                  aria-label="Sign out"
                >
